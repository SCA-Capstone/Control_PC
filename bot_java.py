...
class NextAPIClient:
    ...
    def get_java_jobs(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                submitted_jobs = [
                    job for job in data
                    if job.get('status') == 'submitted' and job.get('config') == 'Java'
                ]
                if submitted_jobs:
                    print(
                        'Submitted jobs with config "Java" retrieved successfully:', submitted_jobs)
                    return submitted_jobs
                else:
                    print('No submitted jobs with config "Java" found.')
                    return None
            else:
                print(
                    f'Error retrieving records: {response.status_code}', response.text)
    ...
    def get_folder(self, folder_id, download_dir='java_files'):
        ...
        for job_id in ids:
            ...
            if compile_result.returncode != 0:
                ...
                with open(stdout_path, 'w') as stdout_file, open(stderr_path, 'w') as stderr_file:
                    ...
                    ["java", "-cp", "java_files", "test"],  # Correct the classpath and class name
                    stdout=stdout_file,
                    stderr=stderr_file,
                    text=True
                )
                ...
                if run_result.returncode == 0:
                    print(f"Program executed successfully. Output saved to {stdout_path} and {stderr_path}.")
                else:
